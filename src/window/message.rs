...
//     state: ButtonState,
//   },
//   /// Raw mouse motion. Use this for mouse input in cases such as
...
//           outer_size: PhysicalSize::new(window_pos.cx as u32, window_pos.cy
// as u32),         })
//       }
//       WindowsAndMessaging::WM_SETFOCUS =>
// out.push(Message::Focus(Focus::Gained)),
//       WindowsAndMessaging::WM_KILLFOCUS =>
// out.push(Message::Focus(Focus::Lost)),       WindowsAndMessaging::WM_COMMAND
// => out.push(Message::Command),       WindowsAndMessaging::WM_SYSCOMMAND =>
// out.push(Message::SystemCommand),       WindowsAndMessaging::WM_DPICHANGED =>
...
//             let button_flags = unsafe {
// mouse_data.Anonymous.Anonymous.usButtonFlags };
...
//             for (id, state) in
// mouse_button_states(button_flags).iter().enumerate() {               if let
// Some(state) = *state {                 let button =
// MouseButton::from_state(id);                 out
//                   .push(Message::RawInput(RawInputMessage::MouseButton {
// button, state }))               }
//             }
//           }
...
//               out.push(Message::RawInput(RawInputMessage::Keyboard { key,
// state }))             }
//           }
//           _ => return None,
//         }
...
//         let kind =
//           get_cursor_move_kind(hwnd, state.read_lock().cursor.inside_window,
// x, y);
...
//               true
//             }
//             CursorMoveKind::Inside => state.read_lock().cursor.last_position
// != position,           }
//         };
...
//     let flags = hi_word(unsafe { std::mem::transmute::<i32, u32>(l_param.0 as
// i32) });

//     let is_extended_key = is_flag_set(flags, WindowsAndMessaging::KF_EXTENDED
// as u16);
...
//       let was_key_down = is_flag_set(flags, WindowsAndMessaging::KF_REPEAT as
// u16);       let is_key_up = is_flag_set(flags, WindowsAndMessaging::KF_UP as
// u16);
...
//   fn new_mouse_button_message(message: u32, w_param: WPARAM, l_param: LPARAM)
// -> Message {     let flags = w_param.0 as u32;
...
// WindowsAndMessaging::XBUTTON1 {             MouseButton::Back
//           } else {
//             MouseButton::Forward
//           }
//         }
...
//         WindowsAndMessaging::WM_LBUTTONDBLCLK |
// WindowsAndMessaging::WM_LBUTTONDOWN           if is_l_down =>
//         {
//           true
//         }
//         WindowsAndMessaging::WM_MBUTTONDBLCLK |
// WindowsAndMessaging::WM_MBUTTONDOWN           if is_m_down =>
//         {
//           true
//         }
//         WindowsAndMessaging::WM_RBUTTONDBLCLK |
// WindowsAndMessaging::WM_RBUTTONDOWN           if is_r_down =>
//         {
//           true
//         }
//         WindowsAndMessaging::WM_XBUTTONDBLCLK |
// WindowsAndMessaging::WM_XBUTTONDOWN           if is_x1_down || is_x2_down =>
//         {
//           true
//         }
//         _ => false,
...
//     let (x, y) = (signed_lo_word(l_param.0 as i32), signed_hi_word(l_param.0
// as i32));
...
//     matches!(self, Message::Key { key: k, state: s, .. } if *k == key && *s
// == state)   }

//   /// Returns `true` if the message matches the supplied mouse button and
// mouse   /// button state
//   pub fn is_mouse_button(&self, button: MouseButton, state: ButtonState) ->
// bool {     matches!(self, Message::MouseButton { button: b, state: s, .. } if
// *b == button && *s == state)   }
