...
// /// Calculate the `pixels_per_point` for a given window, given the current
// egui /// zoom factor
...
//   /// Call this once a graphics context has been created to update the
// maximum   /// texture dimensions that egui will use.
...
//         }
//       }
//       Message::Scroll { delta_x, delta_y } => {
...
//       WindowEvent::Ime(ime) => {
//         // on Mac even Cmd-C is pressed during ime, a `c` is pushed to
// Preedit.         // So no need to check is_mac_cmd.
//         //
//         // How winit produce `Ime::Enabled` and `Ime::Disabled` differs in
// MacOS         // and Windows.
//         //
//         // - On Windows, before and after each Commit will produce an
// Enable/Disabled         // event.
//         // - On MacOS, only when user explicit enable/disable ime. No
...
//         // When pressing the Tab key, egui focuses the first focusable
// element, hence         // Tab always consumes.
...
//         // We will not be given a KeyboardInput event when the modifiers are
// released         // while the window does not have focus. Unset all modifier
// state to be         // safe.
...
// super_;         self.egui_input.modifiers.command = if cfg!(target_os =
// "macos") {           super_
//         } else {
//           ctrl
//         };
...
//   pub fn on_accesskit_action_request(&mut self, request:
// accesskit::ActionRequest) {     self
//       .egui_input
//       .events
...
//   fn on_mouse_wheel(&mut self, window: &Window, delta:
// winit::event::MouseScrollDelta) {     let pixels_per_point =
// pixels_per_point(&self.egui_ctx, window);
...
//           y,
//         }) => {
//           (egui::MouseWheelUnit::Point, egui::vec2(x as f32, y as f32) /
// pixels_per_point)         }
//       };
...
//       // Note: one Mac we already get horizontal scroll events when shift is
// down.       self
//         .egui_input
//         .events
...
// games. For example the       // default keys for the player to move around
// might be the W, A, S, and D keys       // on a US layout. The position of
// these keys is more important than their       // label, so they should map to
// Z, Q, S, and D on an "AZERTY" layout. (This       // value is `KeyCode::KeyW`
// for the Z key on an AZERTY layout.)       physical_key,

//       // Represents the results of a keymap, i.e. what character a certain
// key press       // represents. When telling users "Press Ctrl-F to find",
// this is where we       // should look for the "F" key, because they may have
// a dvorak layout on       // a qwerty keyboard, and so the logical "F"
// character may not be located on the       // physical `KeyCode::KeyF`
// position.       logical_key,
...
//     let physical_key = if let winit::keyboard::PhysicalKey::Code(keycode) =
// *physical_key     {
...
//         // On some platforms we get here when the user presses Cmd-C (copy),
// ctrl-W,         // etc. We need to ignore these characters that are
// side-effects of         // commands. Also make sure the key is pressed (not
// released). On Linux,         // text might contain some data even when the
// key is released.         let is_cmd = self.egui_input.modifiers.ctrl
...
//   fn set_cursor_icon(&mut self, window: &Window, cursor_icon:
// egui::CursorIcon) {     if self.current_cursor_icon == Some(cursor_icon) {
//       // Prevent flickering near frame boundary when Windows OS tries to
// control       // cursor icon for window resizing. On other platforms: just
...
//       }
//     } else {
//       // Remember to set the cursor again once the cursor returns to the
// screen:       self.current_cursor_icon = None;
//     }
//   }
// }
...
//   let inner_rect_px = if let (Some(pos), Some(size)) = (inner_pos_px,
// inner_size_px) {     Some(Rect::from_min_size(pos, size))
//   } else {
//     None
//   };

//   let outer_rect_px = if let (Some(pos), Some(size)) = (outer_pos_px,
// outer_size_px) {     Some(Rect::from_min_size(pos, size))
//   } else {
//     None
//   };
...
//     // It's tempting to do this, but it leads to a deadlock on Mac when
// running     // `cargo run -p custom_window_frame`.
...
// egui::Key::from_name(str.as_str()),     winit::keyboard::Key::Unidentified(_)
// | winit::keyboard::Key::Dead(_) => None,   }
// }

// fn key_from_named_key(named_key: winit::keyboard::NamedKey) ->
// Option<egui::Key> {   use egui::Key;
...
//     // KeyCode::Colon => Key::Colon, // NOTE: there is no physical colon key
...
// fn translate_cursor(cursor_icon: egui::CursorIcon) ->
// Option<winit::window::CursorIcon> {   match cursor_icon {
//     egui::CursorIcon::None => None,

//     egui::CursorIcon::Alias => Some(winit::window::CursorIcon::Alias),
//     egui::CursorIcon::AllScroll =>
// Some(winit::window::CursorIcon::AllScroll),     egui::CursorIcon::Cell =>
// Some(winit::window::CursorIcon::Cell),     egui::CursorIcon::ContextMenu =>
// Some(winit::window::CursorIcon::ContextMenu),     egui::CursorIcon::Copy =>
// Some(winit::window::CursorIcon::Copy),     egui::CursorIcon::Crosshair =>
// Some(winit::window::CursorIcon::Crosshair),     egui::CursorIcon::Default =>
// Some(winit::window::CursorIcon::Default),     egui::CursorIcon::Grab =>
// Some(winit::window::CursorIcon::Grab),     egui::CursorIcon::Grabbing =>
// Some(winit::window::CursorIcon::Grabbing),     egui::CursorIcon::Help =>
// Some(winit::window::CursorIcon::Help),     egui::CursorIcon::Move =>
// Some(winit::window::CursorIcon::Move),     egui::CursorIcon::NoDrop =>
// Some(winit::window::CursorIcon::NoDrop),     egui::CursorIcon::NotAllowed =>
// Some(winit::window::CursorIcon::NotAllowed),
//     egui::CursorIcon::PointingHand =>
// Some(winit::window::CursorIcon::Pointer),     egui::CursorIcon::Progress =>
// Some(winit::window::CursorIcon::Progress),

//     egui::CursorIcon::ResizeHorizontal =>
// Some(winit::window::CursorIcon::EwResize),     egui::CursorIcon::ResizeNeSw
...
// Some(winit::window::CursorIcon::SeResize),     egui::CursorIcon::ResizeSouth
// => Some(winit::window::CursorIcon::SResize),
//     egui::CursorIcon::ResizeSouthWest =>
// Some(winit::window::CursorIcon::SwResize),     egui::CursorIcon::ResizeWest
// => Some(winit::window::CursorIcon::WResize),
//     egui::CursorIcon::ResizeNorthWest =>
// Some(winit::window::CursorIcon::NwResize),     egui::CursorIcon::ResizeNorth
// => Some(winit::window::CursorIcon::NResize),
//     egui::CursorIcon::ResizeNorthEast =>
// Some(winit::window::CursorIcon::NeResize),     egui::CursorIcon::ResizeColumn
...
// Some(winit::window::CursorIcon::VerticalText),     egui::CursorIcon::Wait =>
// Some(winit::window::CursorIcon::Wait),     egui::CursorIcon::ZoomIn =>
// Some(winit::window::CursorIcon::ZoomIn),     egui::CursorIcon::ZoomOut =>
// Some(winit::window::CursorIcon::ZoomOut),   }
// }
...
//       // If `is_viewport_focused` is not checked on x11 the input will be
// permanently       // taken until the app is killed!
...
//           .then_some(PhysicalSize::new(pixels_per_point * s.x,
...
//         s.map(|s| PhysicalSize::new(pixels_per_point * s.x, pixels_per_point
// * s.y)),       ); } ViewportCommand::Resizable(v) => window.set_resizable(v),
//   ViewportCommand::EnableButtons { close, minimized, maximize, } =>
//   window.set_enabled_buttons( if close { WindowButtons::CLOSE } else {
//   WindowButtons::empty() } | if minimized { WindowButtons::MINIMIZE } else {
//   WindowButtons::empty() } | if maximize { WindowButtons::MAXIMIZE } else {
//   WindowButtons::empty() }, ), ViewportCommand::Minimized(v) => {
//   window.set_minimized(v); info.minimized = Some(v); }
//   ViewportCommand::Maximized(v) => { window.set_maximized(v); info.maximized
//   = Some(v); } ViewportCommand::Fullscreen(v) => { window.set_fullscreen(v.
...
// WindowLevel::AlwaysOnBottom,       egui::viewport::WindowLevel::AlwaysOnTop
// => WindowLevel::AlwaysOnTop,       egui::viewport::WindowLevel::Normal =>
// WindowLevel::Normal,     }),
...
// winit::window::ImePurpose::Terminal,       egui::viewport::IMEPurpose::Normal
...
//   let window_builder =
//     create_winit_window_builder(egui_ctx, event_loop,
// viewport_builder.clone());   let window = {
...
//   // We set sizes and positions in egui:s own ui points, which depends on the
// egui   // zoom_factor and the native pixels per point, so we need to know
// that here.   // We don't know what monitor the window will appear on though,
// but   // we'll try to fix that after the window is created in the vall to
//   // `apply_viewport_builder_to_window`.
...
//     .map_or_else(
//       || {
//         log::debug!("Failed to find a monitor - assuming
// native_pixels_per_point of 1.0");         1.0
//       },
...
// WindowLevel::Normal,     })
//     .with_fullscreen(
//       fullscreen.and_then(|e|
// e.then_some(winit::window::Fullscreen::Borderless(None))),     )
...
//     None
//   } else {
//     match winit::window::Icon::from_rgba(icon.rgba.clone(), icon.width,
// icon.height) {       Ok(winit_icon) => Some(winit_icon),
//       Err(err) => {
...
//       let pos = PhysicalPosition::new(pixels_per_point * pos.x,
// pixels_per_point * pos.y);       window.set_outer_position(pos);
//     }
//   }
// }
...
// short_window_event_description(event),   }
// }

// /// Short and fast description of an event.
// /// Useful for logging and profiling.
// pub fn short_window_event_description(event: &winit::event::WindowEvent) ->
// &'static str {   use winit::event::WindowEvent;

//   match event {
//     WindowEvent::ActivationTokenDone { .. } =>
// "WindowEvent::ActivationTokenDone",     WindowEvent::Resized { .. } =>
// "WindowEvent::Resized",     WindowEvent::Moved { .. } =>
// "WindowEvent::Moved",     WindowEvent::CloseRequested { .. } =>
// "WindowEvent::CloseRequested",     WindowEvent::Destroyed { .. } =>
// "WindowEvent::Destroyed",     WindowEvent::DroppedFile { .. } =>
// "WindowEvent::DroppedFile",     WindowEvent::HoveredFile { .. } =>
// "WindowEvent::HoveredFile",     WindowEvent::HoveredFileCancelled { .. } =>
// "WindowEvent::HoveredFileCancelled",     WindowEvent::Focused { .. } =>
// "WindowEvent::Focused",     WindowEvent::KeyboardInput { .. } =>
// "WindowEvent::KeyboardInput",     WindowEvent::ModifiersChanged { .. } =>
// "WindowEvent::ModifiersChanged",     WindowEvent::Ime { .. } =>
// "WindowEvent::Ime",     WindowEvent::CursorMoved { .. } =>
// "WindowEvent::CursorMoved",     WindowEvent::CursorEntered { .. } =>
// "WindowEvent::CursorEntered",     WindowEvent::CursorLeft { .. } =>
// "WindowEvent::CursorLeft",     WindowEvent::MouseWheel { .. } =>
// "WindowEvent::MouseWheel",     WindowEvent::MouseInput { .. } =>
// "WindowEvent::MouseInput",     WindowEvent::TouchpadMagnify { .. } =>
// "WindowEvent::TouchpadMagnify",     WindowEvent::RedrawRequested { .. } =>
// "WindowEvent::RedrawRequested",     WindowEvent::SmartMagnify { .. } =>
// "WindowEvent::SmartMagnify",     WindowEvent::TouchpadRotate { .. } =>
// "WindowEvent::TouchpadRotate",     WindowEvent::TouchpadPressure { .. } =>
// "WindowEvent::TouchpadPressure",     WindowEvent::AxisMotion { .. } =>
// "WindowEvent::AxisMotion",     WindowEvent::Touch { .. } =>
// "WindowEvent::Touch",     WindowEvent::ScaleFactorChanged { .. } =>
// "WindowEvent::ScaleFactorChanged",     WindowEvent::ThemeChanged { .. } =>
// "WindowEvent::ThemeChanged",     WindowEvent::Occluded { .. } =>
