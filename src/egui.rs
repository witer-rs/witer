...
//     // `WindowBuilder::with_position` expects inner position in Macos, and
// outer     // position elsewhere See
// [`winit::window::WindowBuilder::with_position`]     // for details.
...
//       // Mac sometimes has problems restoring the window to secondary
// monitors       // using only `WindowBuilder::with_position`, so we need this
// extra step:       if let Some(pos) = self.outer_position_pixels {
//         window.set_outer_position(winit::dpi::PhysicalPosition { x: pos.x, y:
// pos.y });       }
//     }
//   }

//   pub fn clamp_size_to_sane_values(&mut self, largest_monitor_size_points:
// egui::Vec2) {     use egui::NumExt as _;
...
//       // Make sure we don't try to create a window larger than the largest
// monitor       // because on Linux that can lead to a crash.
...
//   ) {
//     // If the app last ran on two monitors and only one is now connected,
// then     // the given position is invalid.
//     // If this happens on Mac, the window is clamped into valid area.
//     // If this happens on Windows, the window becomes invisible to the user
// 🤦‍♂️     // So on Windows we clamp the position to the monitor it is on.
...
//       clamp_pos_to_monitors(egui_zoom_factor, event_loop, inner_size_points,
// pos_px);     }
...
//   // default to primary monitor, in case the correct monitor was
// disconnected.   let Some(mut active_monitor) = event_loop
//     .primary_monitor()
...
//     window_size_pts * (egui_zoom_factor * active_monitor.scale_factor() as
// f32);   // Add size of title bar. This is 32 px by default in Win 10/11.
//   if cfg!(target_os = "windows") {
...
//   // To get the maximum position, we get the rightmost corner of the display,
// then   // subtract the size of the window to get the bottom right most value
//   // window.position can have.
//   *position_px = position_px.clamp(monitor_position, monitor_position +
// window_size); }
